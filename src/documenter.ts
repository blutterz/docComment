...
function includeTypes() {
    return vs.workspace.getConfiguration().get("docComment.includeTypes", true);
}

function inferTypes() {
    return vs.workspace.getConfiguration().get("docComment.inferTypesFromNames", false);
}

function enableHungarianNotationEvaluation() {
    return vs.workspace.getConfiguration().get("docComment.enableHungarianNotationEvaluation", false);
}
...
export class Documenter implements vs.Disposable {
    ...
    constructor() {
        ...
        this._services = ts.createLanguageService(this._languageServiceHost);
    }
    ...
    private _documentNode(sb: utils.SnippetStringBuilder, node: ts.Node, sourceFile: ts.SourceFile): ts.LineAndCharacter | null {
        switch (node.kind) {
